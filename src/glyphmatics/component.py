...
def _consistent_color_map(
    ...
    A mapping is accepted only when every pixel of a source color maps to exactly
    one target color across all same-shaped training examples.  This gives the
    high-level ARC component a safe, exact color-remap primitive before falling
    back to the broader meta-policy hypothesis search.
    """
...
def _apply_color_map(grid: np.ndarray, mapping: Dict[int, int]) -> np.ndarray:
    ...
    out = original.copy()
    # Always mask against the original grid so chained mappings cannot cascade.
...
class GlyphMaticsEngine:
    ...
    def run_universe(self) -> Dict[str, object]:
        ...
        # Deterministic internal ARC smoke test used by integration diagnostics.
